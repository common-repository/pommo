 ...
 <form action="" method="POST">
	...
	<div class="field">
		...
		<select name="field_type">
			...
			<option value="bigtext">{t}Big Text{/t}</option>
			<option value="number">{t}Number{/t}</option>
			<option value="checkbox">{t}Check Box{/t}</option>
			<option value="multiple">{t}Choose One{/t}</option>
			<option value="multiplemultiple">{t}Choose Many{/t}</option>
...
<div id="demoOrder">
	...
	<div id="demo_{$key}">
		<a href="{$smarty.server.PHP_SELF}?field_id={$key}&delete=TRUE&{"&"|bmAppendPoMMoInstance}&field_name={$demo.name}">
	 	 		<img src="{$url.theme.shared}images/icons/delete.png" border="0"></a>
		<span style="margin-left: 25px;">
		<a href="fields_edit.php?field_id={$key}{"&"|bmAppendPoMMoInstance}">
...
<br>
<div><li>{t escape=no}Names in <strong>bold</strong> appear on the sign-up form.{/t}</li></div>
...
Sortable.create('demoOrder',{tag:'div', handle: 'handle', onUpdate:function(){new Ajax.Updater('ajaxOutput', 'ajax_demoOrder.php{/literal}{"?"|bmAppendPoMMoInstance}{literal}', {onComplete:function(request){new Effect.Highlight('demoOrder',{});}, parameters:Sortable.serialize('demoOrder'), evalScripts:true, asynchronous:true})}});
