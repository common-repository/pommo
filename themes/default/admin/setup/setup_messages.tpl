...
<div id="mainbar">
	...
	<a href="{$url.base}admin/setup/setup_configure.php{"?"|bmAppendPoMMoInstance}">
...
<input type="hidden" name="pk" value="{$smarty.const.poMMo_instance}">
  ...
  <fieldset>
	...
	<fieldset>
    <legend>{t}Notify New Registration{/t}</legend>

		<div class="field">
			<div class="error">{validate id="notify_sub" message=$formError.notify_sub}</div>
			<label for="Notify_sub"><span class="required">{t}Subject:{/t}</span></label>
			<input type="text" class="text" size="32" maxlength="60"
			  name="Notify_sub" value="{$Notify_sub|escape}" id="notify_sub" />
			<div class="notes">{t}(Subject of Sent Email){/t}</div>
		</div>
		
		<div class="field">
			<div class="error">{validate id="notify_msg" message=$formError.notify_msg}</div>
			<label for="Notify_msg"><span class="required">{t}Message:{/t}</span></label>
			<textarea name="Notify_msg" id="notify_msg" rows="5">{$Notify_msg|escape}</textarea>
			<div class="notes">{t}(Use [[url]] for the confirm link at least once){/t}</div>
		</div>
		
		<!-- The notification email is sent to the admin contact and no success message will be displayed -->
		<input type="hidden" id="notify_suc" value="">
		
		<div class="eveninputs">
			<input type="submit" value="{t}Update{/t}">
			<input type="submit" name="restore[notify]" value="{t}Restore to Defaults{/t}">
		</div>
