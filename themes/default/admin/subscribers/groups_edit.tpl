...
<style>
	...
	body.mode-no_header .hide-on-no-header{
...
<a class="hide-on-no-header" href="{$url.base}admin/subscribers/subscribers_groups.php{"?"|bmAppendPoMMoInstance}">
...
<form id="nameForm" name="nameForm" action="" method="POST">
	...
	<div class="field">
                ...
                <tr>
                  <td>{t}Group Name:{/t}</td>
		  <td><input type="text" class="text"  title="{t}type new group name{/t}" maxlength="60" size="30" name="group_name" id="group_name"  value="{$group_name}" /></td>
                </tr>
                <tr>
                  <td valign="top">{t}Filter Logic:{/t}</td>
		  <td>
                     <input type="radio" class="radio" name="FilterLogic" value="all" {if $filter_logic eq 'all'}checked{/if} /> {t}Match ...ALL... Filters{/t}<br>
                     <input type="radio" class="radio" name="FilterLogic" value="any" {if $filter_logic eq 'any'}checked{/if} /> {t}Match ...ANY... Filter{/t} 
                  </td>
                </tr>
                <tr>
                <tr>
                  <td colspan="2"><input class="button" type="submit" name="rename" value="{t}Update{/t}" /></td>
                </tr>
                </table>
	</div>
</form>
...
<div style="border-top: 1px dotted; padding: 5px;">
	<a href="{$smarty.server.PHP_SELF}?filter_id={$filter_id}&delete=TRUE&group_id={$group_id}{if $smarty.request.mode eq 'no_header'}&mode=no_header{/if}{"&"|bmAppendPoMMoInstance}">
	...
	<span style="text-align:left; margin-left: 12px;">
		...
		{elseif $filter.logic == 'not_equal'}
			{t escape=no 1="<strong>`$demos[$filter.field_id].name`</strong>" 2="<em>`$filter.value`</em>}Include subscribers who have %1 not equal to %2{/t}
		...
		{elseif $filter.logic == 'not_true'}
			{t}Include subscribers that didn't check{/t} <strong>{$demos[$filter.field_id].name}</strong>
		{elseif $filter.logic == 'is_true'}
			{t}Include subscribers that checked{/t} <strong>{$demos[$filter.field_id].name}</strong>
		{elseif $filter.logic == 'contains' || $filter.logic == 'contains_multiple'}
			{t escape=no 1="<strong>`$demos[$filter.field_id].name`</strong>" 2="<em>`$filter.value`</em>}Include subscribers who have %1 containing %2{/t}
		{elseif $filter.logic == 'does_not_contain' || $filter.logic == 'does_not_contain_multiple'}
			{t escape=no 1="<strong>`$demos[$filter.field_id].name`</strong>" 2="<em>`$filter.value`</em>}Include subscribers who have %1 not containing %2{/t}
		{elseif $filter.logic == 'starts_with'}
			{t escape=no 1="<strong>`$demos[$filter.field_id].name`</strong>" 2="<em>`$filter.value`</em>}Include subscribers who have %1 starting with %2{/t}
		{/if}
	</span>
</div>
{foreachelse}
...
function updateLogic() {
	poll = new Ajax.Updater(
		...
		{
			asynchronous:true,
			parameters: "field_id="+$('field_id').value+"{/literal}{"&"|bmAppendPoMMoInstance}{literal}"
		}
	);
	hide();
}
...
function updateGroup(curGroup) {
	poll = new Ajax.Updater(
		...
		{
			asynchronous:true,
			parameters: "group_id="+curGroup+"&group_logic="+$('group_logic').value+"{/literal}{"&"|bmAppendPoMMoInstance}{literal}"
		}
	);
	hide();
}
...
function filterUpdate(filter_id,group_id) {
	poll = new Ajax.Updater(
		...
		{
			asynchronous:true,
			parameters: "update=TRUE&filter_id="+filter_id+"&group_id="+group_id+"{/literal}{"&"|bmAppendPoMMoInstance}{literal}"
		}
	);
	hide();
}
