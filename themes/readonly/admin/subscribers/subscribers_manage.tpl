...
<table cellpadding="5" cellspacing="0" border="0" style="text-align:left;">
	...
	<tr bgcolor="{cycle values="#eeeeee,"}">
		...
		<td nowrap>
			{if $table == 'subscribers'}
				<a href="subscribers_mod.php?sid={$item.id}&action=edit&table={$table}&limit={$limit}&order={$order}&orderType={$orderType}&group_id={$group_id}">{t}expand{/t}</a>
			{else}
				&nbsp;
			{/if}
				</td>
		...
		<td nowrap><strong>{$item.email}</strong> <a href='mailto:{$item.email}'><img src="{$url.theme.shared}images/icons/emailicon.gif" class="navimage" border="0" align='baseline'/></a></td>
		{else}
...
<SELECT name="action">
	...
	{if $table == 'subscribers'}
		<option value="edit">{t}Expand{/t} {t}checked Subscribers{/t}</option>
	{/if}
</SELECT>
